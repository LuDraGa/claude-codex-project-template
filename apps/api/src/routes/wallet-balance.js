...
const { handleGetBalance } = require('../wallet/handlers');
const { sendJson } = require('../http/json-response');
const { withErrorHandling } = require('../http/with-error-handling');

function createWalletBalanceRoute({ walletService, jwtSecret }) {
  return withErrorHandling(async (req, res) => {
    ...
    const response = await handleGetBalance({ authContext, walletService });
    sendJson(res, response.status, response.data);
  });
}

module.exports = {
  createWalletBalanceRoute
};
